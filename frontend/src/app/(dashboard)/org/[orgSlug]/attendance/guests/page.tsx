      ...
      <div className="bg-white border-b border-slate-200 pt-6 pb-4 px-4 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Kelola {isSchool ? 'siswa' : 'tamu'} dan hasilkan QR Code absensi</p>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 mt-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50">
            <h3 className="font-bold text-slate-800">Daftar {isSchool ? 'Siswa' : 'Tamu'} ({guests.length})</h3>
          ...
          <div className="flex overflow-x-auto border-b border-slate-200 px-4 pt-2 hide-scrollbar">
            {['Semua', 'Hadir', 'Izin', 'Sakit', 'Alpha'].map(tab => {
              const value = tab === 'Semua' ? 'all' : tab.toLowerCase();
              return (
                <button
                  key={value}
                  onClick={() => setActiveTab(value)}
                  className={`px-4 py-3 font-bold text-sm border-b-2 whitespace-nowrap transition-colors ${
                    activeTab === value
          ...
          <div className="p-4 bg-slate-50/50">
            {loading ? (
              ...
              const filteredGuests = guests.filter(g => {
                ...
                if (activeTab === 'hadir') return g.status === 'present' || g.status === 'late';
                if (activeTab === 'izin') return g.status === 'izin';
                if (activeTab === 'sakit') return g.status === 'sakit';
                if (activeTab === 'alpha') return g.status === 'alpha' || (!g.status && !g.check_in_time);
                return true;
              });
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                          ...
                          {g.status === 'present' ? (
                            <div className="flex items-center gap-1.5 text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full text-xs font-bold border border-emerald-100">
                              ...
                              {g.check_in_time ? new Date(g.check_in_time).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : 'Hadir'}
                            </div>
                          ) : g.status === 'late' ? (
                            <div className="flex items-center gap-1.5 text-amber-600 bg-amber-50 px-3 py-1 rounded-full text-xs font-bold border border-amber-100">
                              ...
                              {g.check_in_time ? new Date(g.check_in_time).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : 'Terlambat'}
                            </div>
                          ) : g.status === 'izin' ? (
                            ...
                            <div className="flex items-center gap-1.5 text-orange-600 bg-orange-50 px-3 py-1 rounded-full text-xs font-bold border border-orange-100">
                              <CheckCircle2 className="w-3.5 h-3.5" />
                              Sakit
                            </div>
                          ) : g.status === 'alpha' ? (
                            <div className="flex items-center gap-1.5 text-rose-600 bg-rose-50 px-3 py-1 rounded-full text-xs font-bold border border-rose-100">
                    ...
                    <table className="w-full text-left border-collapse min-w-[600px]">
                      <thead>
                        <tr className="border-b border-slate-200 bg-slate-100/50">
                          <th className="px-4 py-3 text-sm font-bold text-slate-700">Nama {isSchool ? 'Siswa' : 'Tamu'}</th>
